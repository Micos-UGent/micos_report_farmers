...
```{r include = FALSE}
# 1) Defineer de variabelen die we willen tonen in de tabel en vat deze samen
...
summary_df <- merged_df %>%
  ...
  # selecteer alleen de variabelen van interesse
  select(any_of(vars)) %>%
  mutate(across(everything(), ~ suppressWarnings(as.numeric(.)))) %>%
  
  # Krijg de gemiddelde waarde en standaard error
  summarise(across(everything(), 
                   list(mean = ~ mean(.x, na.rm = TRUE), 
                        se = ~ {n_non_missing <- sum(!is.na(.x))
        if (n_non_missing > 1) sd(.x, na.rm = TRUE) / sqrt(n_non_missing) else NA_real_ }),
                   .names = "{.col}__{.fn}")) %>%
