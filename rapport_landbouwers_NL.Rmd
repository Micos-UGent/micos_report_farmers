...
tab <- gt::gt(gt_df) %>%
  ...
  gt::cols_label(
    Parameter = "Parameter",
    Analyse_resultaat = md("Gemiddelde waarde<br>(gem. ± SE<sup>1</sup>))"),
    Eenheid = "Eenheid",
    A = "A",
    B = "B",
    C = "C"
...
In onderstaande tabel kan u uw waardes terugvinden van de microplastics, macroplastics en ook de algemene afval categorie. Deze laatste categorie omvat al het verzamelde plastic, maar ook nog keramiek, metalen, stenen, rubber, glass en anderen dat we gevonden hebben op het veld tussen locatie A en C ((bak)stenen, metaal, andere, ...). Omdat dit maar één waarde geeft zal u bij waarde per locatie enkel bij A een resultaat zien en bij de andere NVT (niet van toepassing).
...
```{r  include = FALSE}
# --- 2) Alle MiCoS waardes (all plots) ---
all_summary_df <- merged_df %>%
  ...
  left_join(meta, by = "Variabele") %>%
  mutate(
    Alle_MiCoS_waardes = case_when(
      is.na(mean) ~ NA_character_,
      is.na(se) | se == 0 ~ sprintf("%.1f", mean),
      TRUE ~ paste0(sprintf("%.1f", mean), " \u00B1 ", sprintf("%.1f", se))
    )
  ) %>%
  select(Variabele, Alle_MiCoS_waardes)
...
table_df <- summary_df %>%
  ...
  left_join(replicaten_df, by = "Variabele") %>%
  select(Parameter, Mijn_waardes, Alle_MiCoS_waardes, Eenheid, A, C, )
...
tab <- gt::gt(table_df) %>%
  ...
  gt::cols_label(
    Parameter = "Parameter",
    Mijn_waardes = md("Uw waardes<br>(gem. ± SE)"),
    Alle_MiCoS_waardes = md("Alle waardes<br>(gem. ± SE)"),
    Eenheid = "Eenheid",
    A = "A",
    C = "C"
  ...
  gt::cols_width(
    Parameter ~ gt::px(200),
    Mijn_waardes ~ gt::px(180),
    Alle_MiCoS_waardes ~ gt::px(150),  # nieuwe kolom iets breder
    Eenheid ~ gt::px(100)
  ...
  # Voeg voetnoot bij kolomtitel "Mijn waardes" en "Alle MiCoS waardes"
  tab_footnote(
    footnote = "NVT = Niet van toepassing",
    locations = cells_column_labels(columns = c(Mijn_waardes, Alle_MiCoS_waardes))
  ) %>%
...
make_plot <- function(data, yvar, ylimits = NULL, title = NULL, y_breaks = NULL, y_label = NULL) {
  ...
  ggplot() +
    ...
    geom_text(
      data = highlight_df,
      aes(x = "", y = .data[[yvar]],
          label = format(.data[[yvar]], big.mark = " ", scientific = FALSE)),
      nudge_x = 0.30,
      nudge_y = 0,
      size = 3.4,
      fontface = "bold",
    ...
    scale_y_continuous(
      labels = function(x) format(x, big.mark = " ", scientific = FALSE, digits = 0, nsmall = 0),
      breaks = y_breaks
    ) +
...
p_MP <- make_plot(
  ...
  y_label = "Aantal microplastics per kg droge grond"
)
...
p_plastics <- make_plot(
  ...
  y_label = "Aantal macroplastics per hectare"
)
...
p_macromaterials <- make_plot(
  ...
  ylimits = c(0, 4000),
  title = "Totale macromaterialen",
  y_breaks = seq(0, 4000, 1000),
  y_label = "Aantal totale macromaterialen per hectare"
)
